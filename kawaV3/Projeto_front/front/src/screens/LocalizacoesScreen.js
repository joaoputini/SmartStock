...
const LocalizacoesScreen = () => {
  ...
  const [currentLocalizacao, setCurrentLocalizacao] = useState({
    id: null,
    codigo: "",
    descricao: "",
  });
  ...
  const handleSave = async () => {
    ...
    const url = isEditing
      ? `/localizacoes/${currentLocalizacao.id}`
      : "/localizacoes";
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={listStyles.row}>
            <Text style={[listStyles.cell, { flex: 1, textAlign: "left" }]}>
              {item.codigo}
            </Text>
      ...
      >
        <View style={formStyles.modalContainer}>
          <View style={formStyles.modalView}>
            ...
            <TextInput
              placeholder="Código (Ex: A01-P01)"
              style={formStyles.input}
              value={currentLocalizacao.codigo}
              onChangeText={(text) =>
                setCurrentLocalizacao({ ...currentLocalizacao, codigo: text })
              }
            />
            <TextInput
              placeholder="Descrição"
              style={formStyles.input}
              value={currentLocalizacao.descricao}
              onChangeText={(text) =>
                setCurrentLocalizacao({
                  ...currentLocalizacao,
                  descricao: text,
                })
              }
